...
export function PersonCompanyFormDialog({
  ...
  const people = useQuery(api.people.list, personId ? "skip" : {}) ?? []
  ...
  }, [relationship, relationshipId, personId, companyId, form])

  const onSubmit = async (data: PersonCompanyFormData) => {
    try {
      const submitPersonId = (personId || data.personId) as Id<"people">
      if (relationshipId) {
