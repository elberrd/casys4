...
export function IndividualProcessFormPage({
  ...
  const [quickUserApplicantDialogOpen, setQuickUserApplicantDialogOpen] =
    useState(false);
  const [quickCompanyApplicantDialogOpen, setQuickCompanyApplicantDialogOpen] =
    useState(false);
  const [quickConsulateDialogOpen, setQuickConsulateDialogOpen] =
    useState(false);
  const [hasInitializedForm, setHasInitializedForm] = useState(false);
  const [previousProcessTypeId, setPreviousProcessTypeId] =
    useState<string>("");
  ...
  const personPassports =
    useQuery(
      api.passports.listByPerson,
      selectedPersonId
        ? { personId: selectedPersonId as Id<"people"> }
        : "skip",
    ) ?? [];
  ...
  useEffect(() => {
    ...
    if (individualProcess && !hasInitializedForm) {
      ...
      if (
        currentValues.companyApplicantId !==
        (individualProcess.companyApplicantId ?? "")
      ) {
        updates.companyApplicantId = individualProcess.companyApplicantId ?? "";
      }
      if (
        currentValues.userApplicantId !==
        (individualProcess.userApplicantId ?? "")
      ) {
      ...
      if (
        currentValues.processTypeId !== (individualProcess.processTypeId ?? "")
      ) {
        updates.processTypeId = individualProcess.processTypeId ?? "";
      }
      if (
        currentValues.legalFrameworkId !==
        (individualProcess.legalFrameworkId ?? ("" as Id<"legalFrameworks">))
      ) {
        updates.legalFrameworkId =
          individualProcess.legalFrameworkId ?? ("" as Id<"legalFrameworks">);
      }
      ...
      if (
        ...
        updates.qualification = (individualProcess.qualification ?? "") as
          | ""
          | "medio"
          | "tecnico"
          | "mestrado"
      ...
      if (
        currentValues.professionalExperienceSince !==
        (individualProcess.professionalExperienceSince ?? "")
      ) {
        updates.professionalExperienceSince =
          individualProcess.professionalExperienceSince ?? "";
      }
      ...
      if (
        currentValues.deadlineDate !== (individualProcess.deadlineDate ?? "")
      ) {
        updates.deadlineDate = individualProcess.deadlineDate ?? "";
      }
      if (
        currentValues.deadlineUnit !== (individualProcess.deadlineUnit ?? "")
      ) {
        updates.deadlineUnit = individualProcess.deadlineUnit ?? "";
      }
      if (
        currentValues.deadlineQuantity !== individualProcess.deadlineQuantity
      ) {
        updates.deadlineQuantity = individualProcess.deadlineQuantity;
      }
      if (
        currentValues.deadlineSpecificDate !==
        (individualProcess.deadlineSpecificDate ?? "")
      ) {
        updates.deadlineSpecificDate =
          individualProcess.deadlineSpecificDate ?? "";
      }
  ...
  useEffect(() => {
    ...
    // Keep an explicitly selected passport (for example one just created from
    // the quick passport flow) when it belongs to the selected person.
    const currentPassportId = form.getValues("passportId");
    if (
      currentPassportId &&
      personPassports.some((passport) => passport._id === currentPassportId)
    ) {
      return;
    }
  ...
  useEffect(() => {
    ...
    if (
      previousProcessTypeId &&
      previousProcessTypeId !== selectedProcessTypeId
    ) {
  ...
  const handleQuickPersonSuccess = (
    ...
    if (passportId) {
      form.setValue("passportId", passportId);
    }
