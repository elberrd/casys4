/**
 * Countries Phone Data
 *
 * Comprehensive list of countries with phone number formatting information.
 * Includes ISO 3166-1 alpha-2 country codes, country names, dial codes,
 * flag emojis, and phone number format masks.
 *
 * Data organized alphabetically by country name for easy maintenance.
 */

export interface Country {
  code: string; // ISO 3166-1 alpha-2 country code
  name: string; // Country name in English
  dialCode: string; // International dialing code with + prefix
...
export const countries: Country[] = [
  ...
  { code: 'DO', name: 'Dominican Republic', dialCode: '+1-809', flag: '🇩🇴', format: '(###) ###-####' },
  ...
  { code: 'JM', name: 'Jamaica', dialCode: '+1-876', flag: '🇯🇲', format: '(###) ###-####' },
  ...
  { code: 'TT', name: 'Trinidad and Tobago', dialCode: '+1-868', flag: '🇹🇹', format: '(###) ###-####' },
...
export function findCountryByDialCode(dialCode: string): Country | undefined {
  // Normalize dial code by ensuring it starts with +
  const normalizedDialCode = dialCode.startsWith('+') ? dialCode : `+${dialCode}`;
  return countries.find((c) => c.dialCode === normalizedDialCode);
}
 ...
 * If no format is specified, returns the raw number
 */
export function formatPhoneNumber(phoneNumber: string, country: Country): string {
  if (!country.format || !phoneNumber) {
    return phoneNumber;
  }
