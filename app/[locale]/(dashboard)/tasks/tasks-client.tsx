...
import { getSaoPauloDate } from "@/lib/notification-targets";
...
export function TasksClient() {
  ...
  const [selectedTaskForReassign, setSelectedTaskForReassign] =
    useState<Id<"tasks"> | null>(null);
  const [extendDeadlineDialogOpen, setExtendDeadlineDialogOpen] =
    useState(false);
  const [selectedTaskForExtend, setSelectedTaskForExtend] =
    useState<Id<"tasks"> | null>(null);
  ...
  const [selectedTasksForBulkReassign, setSelectedTasksForBulkReassign] =
    useState<any[]>([]);
  const [bulkStatusUpdateDialogOpen, setBulkStatusUpdateDialogOpen] =
    useState(false);
  const [
    selectedTasksForBulkStatusUpdate,
    setSelectedTasksForBulkStatusUpdate,
  ] = useState<any[]>([]);
  ...
  const myTasksInProgress = useQuery(api.tasks.getMyTasks, {
    status: "in_progress",
  });
  ...
  const weekFromNow = getSaoPauloDate(Date.now() + 7 * 24 * 60 * 60 * 1000);
  ...
  useEffect(() => {
    if (highlightTaskId && myTasks && highlightTaskId !== activeHighlightId) {
      ...
      const taskInOverdue = myTasksOverdue?.find(
        (t) => t._id === highlightTaskId,
      );
    ...
    <>
      ...
      <div className="flex flex-1 flex-col gap-4 p-4 pt-0">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>{t("tasksOverview")}</CardDescription>
          </CardHeader>
          <CardContent>
      ...
      {selectedTaskForReassign && (
        <ReassignTaskDialog
          ...
          currentAssignee={
            (allTasks || []).find(
              (task) => task._id === selectedTaskForReassign,
      ...
      {selectedTaskForExtend && (
        <ExtendDeadlineDialog
          ...
          currentDueDate={
            [...(myTasks || []), ...(allTasks || [])].find(
              (task) => task._id === selectedTaskForExtend,
            )?.dueDate || ""
          }
          onSuccess={() => {
      ...
      {selectedTaskForStatusUpdate && (
        <TaskStatusUpdateDialog
          ...
          currentStatus={
            ([...(myTasks || []), ...(allTasks || [])].find(
              (task) => task._id === selectedTaskForStatusUpdate,
