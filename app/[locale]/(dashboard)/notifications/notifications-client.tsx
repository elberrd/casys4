...
export function NotificationsClient() {
  ...
  const snoozeNotification = useMutation(api.notifications.snooze);
  ...
  const handleBulkDelete = async (ids: Id<"notifications">[]) => {
    try {
      await Promise.all(
        ids.map((id) => deleteNotification({ notificationId: id })),
      );
      toast({
        title: tCommon("success"),
  ...
  const handleSnooze = async (id: Id<"notifications">, hours: 2 | 5) => {
    try {
      await snoozeNotification({ notificationId: id, hours });
      toast({
        title: tCommon("success"),
        description: t("snoozedSuccess", { hours }),
      });
    } catch {
      toast({
        title: tCommon("error"),
  ...
  const breadcrumbs = [{ label: t("notifications"), href: "/notifications" }];
