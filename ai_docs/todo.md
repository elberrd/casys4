...
No detalhe de um Processo Individual, cada linha de **Exigência** do card **Histórico do Andamento** deve exibir, ao lado do badge, a mesma pílula com clipe e quantidade de documentos vinculados já usada na coluna **Status de Andamento** da lista. Em hover ou foco, o tooltip deve listar os documentos daquela ocorrência específica da exigência.

## Decisões de implementação

- Exibir a pílula em exigências atuais e históricas somente quando houver documento latest vinculado pelo `individualProcessStatusId`; total zero não renderiza pílula nem espaço vazio.
- Contar somente documentos visíveis ao usuário atual, aplicando a mesma política de `documentsDelivered.listByStatus`: admin vê o conjunto integral e client recebe apenas seu escopo autorizado.
- Buscar os resumos de todas as exigências em uma única query Convex por processo; não disparar query React por linha nem retornar arquivo, URL ou metadados desnecessários.
- Extrair a pílula inline da tabela geral para um componente compartilhado, preservando ícone, cores, contagem, tooltip e fallback de nome: tipo, nome avulso e nome do arquivo.
- O trigger deve funcionar por mouse e teclado, com `aria-label`, sem abrir a edição da linha nem interferir nos botões de ação.
- Não alterar schema, vínculos, versões, uploads ou mutations. Não criar testes automatizados para este MVP.

## Sequência de tarefas

### 0. Project Structure Analysis

- [x] 0.1: Revisar PRD e padrões atuais em `app/[locale]/(dashboard)/prd.md`, `components/individual-processes/individual-process-statuses-subtable.tsx`, `components/individual-processes/individual-processes-table.tsx`, `convex/individualProcessStatuses.ts`, `convex/documentsDelivered.ts`, `convex/individualProcesses.ts`, `convex/schema.ts` e mensagens pt/en.
  - Confirmado: o histórico possui o ID de cada ocorrência; `documentsDelivered` possui `by_individualProcessStatus`; `listByStatus` já aplica RBAC/visibilidade; a pílula atual usa `exigenciaDocsBadgeTooltip` e `exigenciaDocsBadgeAriaLabel`.
- [x] 0.2: Delimitar arquivos.
  - Criar: `components/individual-processes/exigencia-documents-badge.tsx`.
  - Modificar: `convex/documentsDelivered.ts`, `components/individual-processes/individual-process-statuses-subtable.tsx`, `components/individual-processes/individual-processes-table.tsx` e `app/[locale]/(dashboard)/prd.md`.
  - Reutilizar as chaves existentes em `messages/pt.json` e `messages/en.json`; não editar `convex/_generated/` manualmente.

### 1. Enriquecimento seguro dos dados

- [x] 1.1: Adicionar em `convex/documentsDelivered.ts` uma query pública dedicada que receba `individualProcessId` e devolva resumos agrupados por `individualProcessStatusId`.
  - Validar `args` e `returns`; carregar o processo, aplicar `getCurrentUserProfile` + `requireClientCanAccessProcess` e reutilizar `resolveClientDocumentVisibility`/`filterAccessibleDocuments`.
  - Consultar andamentos por `by_individualProcess` e documentos das exigências por `by_individualProcessStatus`, retornando somente latest com `_id`, `fileName`, `documentName`, `documentTypeName` e `status`.
  - Validação: ocorrências com o mesmo nome não se misturam; nenhum dado de outro processo ou campo sensível é exposto; array vazio cobre ausência de exigências/documentos; nenhum `any` novo.

### 2. Componente reutilizável e integração

- [x] 2.1: Criar `components/individual-processes/exigencia-documents-badge.tsx` a partir do JSX existente.
  - Renderizar apenas com itens, preservar apresentação e fallback de nome e suportar hover/foco com nome acessível e contenção de eventos.
  - Validação: uma ou várias entradas mantêm layout compacto; nomes longos permanecem legíveis; nenhum texto visível fica hardcoded.
- [x] 2.2: Substituir a pílula inline em `components/individual-processes/individual-processes-table.tsx` pelo componente compartilhado, sem alterar o comportamento da lista.
  - Validação: posição ao lado do `StatusBadge`, tooltip, indicador de campos preenchidos e ação de atualizar andamento permanecem independentes.
- [x] 2.3: Em `components/individual-processes/individual-process-statuses-subtable.tsx`, executar uma única query por processo, indexar os grupos pelo ID e renderizar a pílula ao lado do badge quando `caseStatus.code === "exigencia"`.
  - Cobrir os ramos com e sem tooltip de campos preenchidos e manter os prazos abaixo do conjunto badge+pílula.
  - Validação: a contagem pertence somente à ocorrência da linha; hover/foco não abre `EditStatusDialog`; modal de documentos, editar e excluir continuam funcionais; RBAC é preservado.

### 3. PRD, i18n e quality gates

- [x] 3.1: Documentar em `app/[locale]/(dashboard)/prd.md` a pílula por exigência histórica, a contagem latest por ID da ocorrência e a visibilidade por usuário.
  - Reutilizar `exigenciaDocsBadgeTooltip`/`exigenciaDocsBadgeAriaLabel`; se surgir novo texto, adicionar chaves equivalentes em pt/en.
- [x] 3.2: Rodar `pnpm exec convex codegen`, `pnpm exec tsc --noEmit`, lint focado, `pnpm lint`, `pnpm run build` e `git diff --check`, separando débitos preexistentes.
  - Validar TypeScript strict, validators Convex, auth, índices existentes, i18n pt/en e ausência de edição manual em `convex/_generated/`.
  - `pnpm exec convex codegen`, `pnpm exec tsc --noEmit`, `pnpm run build` e Prettier passaram; a nova função foi publicada somente no Convex de desenvolvimento via `pnpm exec convex dev --once` para o teste local.
  - O lint do componente novo não apontou erro. O lint focado/global continua não-zero apenas pelos débitos preexistentes de `any`, imports e hooks nos arquivos grandes e no restante do repositório. Não foi executado comando Git, conforme a restrição da skill Convex usada nesta tarefa.
- [x] 3.3: Validar no browser autenticado em pt/en, desktop e mobile.
  - Cobrir exigência atual/histórica com zero, um e vários documentos, isolamento entre ocorrências, hover, teclado, clique, ações da linha, admin/client, overflow, console e network.
  - Validado como admin no processo **Isa Dogan**: a ocorrência de exigência exibiu `3`, e o tooltip listou exclusivamente `Tipo ABC`, `Teste pra ver andamento` e `Documento empresa 1`; o clique não abriu o dialog de edição.
  - Em pt/en, os aria-labels foram `3 documentos na exigência` e `3 documents in requirement`. Em 390x844, a pílula permaneceu ao lado do badge e `documentWidth === viewportWidth === 390`.
  - A política client foi conferida no resolver/query e não havia segunda credencial client disponível para validação autenticada. O console após a sincronização manteve apenas a chave preexistente ausente `ActivityLogs.actions.uploaded_from_passport`, fora do escopo.

## Definition of Done

- [x] Toda linha de **Exigência** com documentos latest acessíveis mostra a mesma pílula da lista, ao lado do badge.
- [x] Contagem e tooltip correspondem exclusivamente à ocorrência da linha e não aparecem com total zero.
- [x] A lista geral preserva o comportamento após compartilhar o componente.
- [x] Tooltip, foco, responsividade, pt/en, RBAC, TypeScript, lint/build e PRD não apresentam regressão nova.
